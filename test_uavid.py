...
def get_img_padded(image, patch_size):
    oh, ow = image.shape[0], image.shape[1] # 3840, 2160
    ...
    h, w = oh + height_pad, ow + width_pad # 3840+
...
def make_dataset_for_one_huge_image(img_path, patch_size):
    img = cv2.imread(img_path, cv2.IMREAD_COLOR) # 4k (3840*2160)
...
def main():
    ...
    model = Supervision_Train.load_from_checkpoint(os.path.join(config.weights_path, config.test_weights_name+'.ckpt'), config=config,map_location="cpu")
